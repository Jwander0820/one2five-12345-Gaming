      ...
      for (let i = 0; i < areaCards.length; i++) {
        if (!areaCards[i].hasChildNodes()) {
          ...
          if (playHistory.player2History.length === 5) {
            let data = resolveGameResult(playHistory);
            console.log(data)
          }
          break;
        }
...
function resolveGameResult(data) {
  let player1History = data.player1History.map(history => parseInt(history.split("_")[history.split("_").length - 1][history.split("_")[history.split("_").length - 1].length - 1]));
  let player2History = data.player2History.map(history => parseInt(history.split("_")[history.split("_").length - 1][history.split("_")[history.split("_").length - 1].length - 1]));

  data.player1History = player1History;
  data.player2History = player2History;

  let GameResult = processGameResult(player1History, player2History);
...
function processGameResult(player1, player2) {
  ...
  for (let i = 0; i < 5; i++) {
      if (player1[i] === player2[i]) {
          continue;
      } else if ((player1[i] === 1 && player2[i] === 5) || (player1[i] === 2 && player2[i] === 4)) {
          player1Score++;
      } else if ((player1[i] === 5 && player2[i] === 1) || (player1[i] === 4 && player2[i] === 2)) {
          player2Score++;
