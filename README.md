...
### A. 直接玩（GitHub Pages 靜態版）
打開：<https://jwander0820.github.io/one2five-12345-Gaming/frontend/12345.html>
...
```
瀏覽器打開：<http://localhost:8080/12345.html>
...
## 靜態版與未來擴充策略
目前 `frontend/12345.html + 12345.js + 12345.css` 是**純前端靜態版**，可以直接部署在 GitHub Pages。

後續若要上正式網域並串接後端，建議保留同一份前端 UI，新增 API adapter：
- 儲存對戰紀錄到資料庫
- 網路即時對戰（WebSocket）
- AI 對弈（HTTP API）

也就是：**先做純前端可玩版本，再接後端能力**，可避免早期開發卡在部署與跨域。
